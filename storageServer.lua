...
settings.define("exportChests", {
    description = "The peripheral name of the export chest",
    {"minecraft:chest_0"},
    type = "table"
})
settings.define("importChests", {
    description = "The peripheral name of the import chests",
    default = {"minecraft:chest_2"},
    type = "table"
})
...
local function printClients()
    ...
    for i in pairs(clients) do
        print(tostring(clients[i].username) .. ":" .. string.sub(tostring(clients[i].target), 1, 5) .. ":" ..
                  tostring(clients[i].sender))
    end
    print("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")
end
...
local function reloadStorageDatabase(reason)
    ...
    if peripheral.find("rs_bridge") == nil then
        -- Refined Storage is not present, use the normal method to get the item list
        storages = getStorage()
        -- This part is slow
...
local function patchStorageDatabase(inputItem, count, chest, slot, reason)
    if reason == nil then
        reason = "unknown"
    end
    ...
    if next(savedDetails) then
        ...
        if chest ~= nil and slot ~= nil then
            ...
            if chestP ~= nil then
                ...
                if chestSize ~= nil and slot <= chestSize then
                    local slotDetails = chestP.getItemDetail(slot)
                    if slotDetails ~= nil then
                        local tmp = {}
                    ...
                    print("chestSize of " .. tostring(chestSize) .. " but tried to index slot " .. tostring(slot))
                    debugLog("chestSize of " .. tostring(chestSize) .. " but tried to index slot " .. tostring(slot))
                end
            end
        end
    end
    ...
    reloadStorageDatabase(tostring(reason) .. ", patchStorageDatabase failed")
    return false
end
...
local function getItem(requestItem, chest)
    ...
    if peripheral.find("rs_bridge") == nil then
                ...
                if chestP ~= nil and
                    ...
                    if requestItem.nbt == nil and item.nbt == nil then
                        if item.count >= amount then
                            ...
                            local patchstatus = patchStorageDatabase(item, -1 * moved, item.chestName, item.slot, "getItem(), item.count >= amount")
                            return
                        else
                            -- print("Export: " .. requestItem.name .. " #" .. tostring(amount))
                            debugLog(("Export: " .. requestItem.name .. " #" .. tostring(amount) .. " chest:" ..
        ...
        if moved > 0 then
            local patchstatus = patchStorageDatabase(requestItem, -1 * moved, nil, nil, "getItem(), RS system used")
        end
    end
...
local function mergeCraftingQueue()
    ...
    -- Add refined storage queue if rs_bridge is present
    local rsBridge = peripheral.find("rs_bridge")
    if rsBridge ~= nil then
        local craftingTasks = rsBridge.getCraftingTasks()
        if type(craftingTasks) == "table" then
            for _, task in pairs(craftingTasks) do
                local rsItem = {}
                rsItem.name = task.resource.name
                rsItem.displayName = task.resource.displayName or task.resource.name
                rsItem.amount = task.quantity
                rsItem.count = task.quantity
                rsItem.recipeType = "rs_crafting"
                rsItem.recipeInput = "rs"
                rsItem.recipe = {}
                rsItem.recipeName = "RS_" .. task.resource.name
                
                table.insert(queue, rsItem)
            end
        end
    end
    ...
    while true do
        ...
        for monitorid = 1, #monitors, 1 do
            ...
            if monitor ~= nil then
                ...
                centerText(monitor, tostring(storageUsed) .. "/" .. tostring(storageMaxSize) .. " Items " ..
                    tostring(("%.3g"):format(percent) .. "%"))
                ...
                centerText(monitor,
                    tostring(storageTotalSlots - storageFreeSlots) .. "/" .. tostring(storageTotalSlots) .. " Slots " ..
                        tostring(("%.3g"):format(percent) .. "%"))
                ...
                if craftingEnabled and mainCraftingServer ~= nil then
                    ...
                    for k, v in pairs(queue) do
                        if k < (height - 15) and v ~= nil then
                            ...
                            text = v.displayName .. ": #" .. tostring(v.amount) .. " - " .. v.recipeName
            ...
            for i, item in pairs(list) do
                ...
                if peripheral.find("rs_bridge") ~= nil then
                    ...
                    if chest == nil then
                        ...
                        if moved > 0 then
                            local patchstatus = patchStorageDatabase(item, moved, chest, slot, "importHandler: item moved from " .. item.chestName .. " slot " .. tostring(item.slot) .. " to " .. chest .. " slot " .. tostring(slot))
...
local function onCryptoNetEvent(event)
    ...
    elseif event[1] == "encrypted_message" then
        ...
        if socket ~= nil and
            (socket.username ~= nil or
                (not settings.get("requireLogin") and socket.sender == settings.get("serverName"))) and event[2][1] ~=
            "hashLogin" then
                ...
                if next(list) then
                    ...
                    for i, item in pairs(list) do
                        ...
                        if chest == nil then
                            ...
                            local moved = peripheral.wrap(item.chestName).pushItems(chest, item.slot, item.count, slot)
                            if moved > 0 then
                                local patchstatus = patchStorageDatabase(item, moved, chest, item.slot, "forceImport: item moved from " .. item.chestName .. " slot " .. tostring(item.slot) .. " to " .. chest .. " slot " .. tostring(slot))
                                if not patchstatus then
                                    reload = true
                                end
                                print("Import: " .. item.name .. " #" .. tostring(moved))
                                debugLog(
                                    "forceImport: " .. item.name .. " #" .. tostring(moved) .. " chest:" .. chest ..
                                        " slot:" .. tostring(slot))
                            end
                        end
                    end
            ...
            elseif message == "setPermissionLevel" then
                ...
                if permissionLevel >= 2 and userExists and type(data.permissionLevel) == "number" and
                    data.permissionLevel < 3 then
            ...
            elseif message == "checkPasswordHashed" then
                ...
                if (not settings.get("requireLogin") and socket.sender == settings.get("serverName")) or
                    tonumber(permissionLevel) >= 2 then
            ...
            elseif message == "setPassword" then
                ...
                local userExists = cryptoNet.userExists(data.username, serverLAN)
                debugLog(
                    "setPassword:" .. socket.username .. ":" .. data.username .. ":" .. tostring(permissionLevel) .. ":" ..
                        tostring(userExists))
            ...
            elseif message == "getCraftingQueue" then
                ...
                debugLog("gotCraftingQueue: " .. dump(data))
                craftingQueue = data
            elseif message == "pushCurrentlyCrafting" then
                debugLog("pushCurrentlyCrafting: " .. dump(data))
                currentlyCrafting = data
            elseif message == "pushCraftingQueue" then
                debugLog("pushCraftingQueue: " .. dump(data))
                craftingQueue = data
            ...
            elseif message == "pullItems" then
                ...
                local patchstatus = patchStorageDatabase(data, -1 * itemsMoved, data.chestName, data.slot, "pullItems: item moved from " .. data.chestName .. " slot " .. tostring(data.slot) .. " to " .. data.craftingChest .. " slot " .. tostring(data.slot))
            elseif message == "patchStorageDatabase" then
                local patchstatus = patchStorageDatabase(data, data.count, data.chestName, data.slot, "patchStorageDatabase: message from " .. socket.username)
            end
    ...
    elseif event[1] == "connection_closed" then
        ...
        log("connection closed: " .. tostring(socket.username) .. ":" .. string.sub(tostring(socket.target), 1, 5) ..
                ":" .. tostring(socket.sender))
        ...
        for i in pairs(clients) do
            if clients[i].target == socket.target then
                ...
                print("Client Disconnected: " .. tostring(socket.username) .. ":" ..
